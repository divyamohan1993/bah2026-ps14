...
| TFT (demo, 4 epochs) | −0.945 | −0.880 | −0.609 | 0.446 | 0.590 |
...
- **The TFT in this CPU demo is under-trained (4 epochs on a small subset) and does *not* beat the
  baselines** (negative PE). It is included to exercise the full dual-head quantile + exceedance
  training/serving path; reaching its potential needs the full-scale config (longer history, more
  epochs, GPU) documented in `config/default.yaml` and `ARCHITECTURE.md`. This is a known
  limitation of the *demo*, not of the pipeline.
